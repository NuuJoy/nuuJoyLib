...
def lossfunc_bce(pred,trgt,eps=1e-16):
...
class relu2sigm2bcel(object):
    ...
    def pack_model_state(self):
        ...
        for optimname in ('hdn_weight_optimizr','hdn_bias_optimizr','out_weight_optimizr','out_bias_optimizr'):
            ...
            for optimattr in ('iter_count','beta1','beta2','eps'):
...
class adam(gradient):
    def __init__(self,learn_rate=0.1,decay_step=128,decay_rate=0.9,beta1=0.9,beta2=0.999):
        ...
        self.vtn1 = 0.0
    def update_params(self,params,dcostdpara):
        numpy.seterr(all='ignore') # to handle numpy underflow
        self.iter_count += 1
        mt_corr = 1 - self.beta1**self.iter_count
        vt_corr = 1 - self.beta2**self.iter_count
        mt = numpy.nan_to_num(self.beta1*self.mtn1 + (1-self.beta1)*dcostdpara)
        vt = numpy.nan_to_num(self.beta2*self.vtn1 + (1-self.beta2)*dcostdpara**2)
        self.mtn1 = numpy.nan_to_num(numpy.mean(mt,axis=0))
        self.vtn1 = numpy.nan_to_num(numpy.mean(vt,axis=0))
